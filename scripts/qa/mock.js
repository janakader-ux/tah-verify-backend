...
window.fetch=(url,options={})=>{
 ...
 if(path.endsWith('/submitted'))return response({submitted:true});
 if(path.endsWith('/payment'))return review==='ready'?response({checkout_id:'cs_dummy_'+app.case_ref,hosted_checkout_url:location.origin+'/__qa__/checkout.html?case='+encodeURIComponent(app.case_ref),provider:'dummy'}):response({detail:'Review required'},409);
...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 panel.innerHTML='<h2>Isolated workflow test — no real payments, emails or ID checks</h2><p>Use synthetic data only. These controls are absent from production.</p><button type="button" id="qaFill">Fill synthetic applicant fields</button> <button type="button" id="qaDocument">Use synthetic document image</button> <button type="button" id="qaApprove">Approve test review</button><p id="qaStatus" role="status"></p><details><summary>Test request audit</summary><pre id="qaAudit"></pre></details>';
 ...
 document.getElementById('qaDocument').onclick=()=>{const canvas=document.createElement('canvas');canvas.width=600;canvas.height=450;const ctx=canvas.getContext('2d');ctx.fillStyle='white';ctx.fillRect(0,0,600,450);ctx.fillStyle='black';ctx.font='28px sans-serif';ctx.fillText('SYNTHETIC TEST - NOT AN ID',40,220);canvas.toBlob(blob=>{const dt=new DataTransfer();dt.items.add(new File([blob],'synthetic-test.jpg',{type:'image/jpeg'}));const input=document.getElementById('precheckFiles');input.files=dt.files;input.dispatchEvent(new Event('change',{bubbles:true}));document.getElementById('qaStatus').textContent='Synthetic file selected. Use the normal Upload button.';},'image/jpeg');};
 document.getElementById('qaApprove').onclick=()=>{if(review!=='manual'){document.getElementById('qaStatus').textContent='Upload a synthetic document first.';return;}review='ready';document.getElementById('qaStatus').textContent='Synthetic review approved. Use Check review status in the application.';};
});
})();
