...
for i,(res,rt,exp) in enumerate([("uk","online",49.0),("uk","in-person",125.0),
                                 ("overseas","online",119.0),("overseas","in-person",125.0)]):
...
check("tampered overseas -> forced to 119", r.json().get("fee_amount"), 119.0)
...
check("penny attack -> forced to 119", r.json().get("fee_amount"), 119.0)

r = post("ATK4", "overseas", "online", -119.0)
check("negative amount -> forced to 119", r.json().get("fee_amount"), 119.0)
...
check("no fee_amount sent", r.json().get("fee_amount"), 119.0)
...
r = post("LEG2", "overseas", None, 119.0)
check("overseas, no route -> highest tier 125", r.json().get("fee_amount"), 125.0)
...
check("stored 1.00 but charge_amount_for", A.charge_amount_for(rec), 119.0)
check("stripe unit_amount (pence)", int(round(A.charge_amount_for(rec)*100)), 11900)
